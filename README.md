...
安装时 Chrome 会展示本扩展需要访问的网站范围，主要包括即梦、Gemini 以及清单中列出的本地开发地址。若你的 PicPuck 管理端部署在其它 HTTPS 域名，请在本地修改 `manifest.json`：在 `host_permissions` 与第一个 `content_scripts` 的 `matches` 中追加该来源，保存后在 `chrome://extensions` 中重新加载扩展。
