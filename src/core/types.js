 ...
 * @property {boolean} [recoverAllocateSilentDefault] 为 true 时：异步找回「检查」阶段默认静默分配 Tab（新建 `active:false`、复用不抢焦点）；用户可将 `picpuckRecoverCheckFocusTab` 置 true 以在检查阶段也激活工作 Tab。「取回」就绪后由业务步骤显式 `focusWorkTab`。
 */
